...
{
  TEST_EQUAL ("frame_index size() matches",
              a->size(), b->size() );
  TEST_EQUAL("frame_index empty() matches",
             a->empty(), b->empty());
  TEST_EQUAL("frame_index first_frame() matches",
             a->first_frame(), b->first_frame());
  TEST_EQUAL("frame_index last_frame() matches",
             a->last_frame(), b->last_frame());

  auto all_frames_s = a->all_frame_ids();
  auto all_frames_f = b->all_frame_ids();
  TEST_EQUAL("frame_index all_frame_ids() matches",
  ...
  TEST_EQUAL("frame_index all_track_ids() matches",
  ...
  TEST_EQUAL("frame_index active_tracks() matches",
  ...
  TEST_EQUAL("frame_index inactive_tracks() matches",
  ...
  TEST_EQUAL("frame_index new_tracks() matches",
  ...
  TEST_EQUAL("frame_index terminated_tracks() matches",
             std::equal(term_s.begin(), term_s.end(),
                        term_f.begin()), true);

  TEST_EQUAL("frame_index percentage_tracked() matches",
